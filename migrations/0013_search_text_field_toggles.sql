drop function if exists public.submissions_search(text, text, text[], text, int, int);
drop function if exists public.submissions_search_scored(text);

create or replace function public.submissions_search_scored(
  q          text,
...
begin
  ...
  for i in 1 .. array_length(toks, 1) loop
    ...
    if incl_desc then
      ids := ids || array(
        select s.id from submissions s where s.project_description ilike pat
      );
    end if;

    if incl_tasks then
      ids := ids || array(
        select d.submission_id from submission_detail_stages d where d.description ilike pat
  ...
  cap := greatest(200, (reltuples / 8)::int)
  ...
  b as (
    select s.id,
           ...
           case when incl_desc then lower(coalesce(s.project_description, '')) else '' end as b_desc,
    ...
    left join lateral (
      ...
      where incl_tasks and d.submission_id = s.id
    ) st on true
    where s.id = any (ids)
  ...
  f as (
    select b.id,
           ...
           avg(case when b.b_desc  = '' then 0 when position(k.tok in b.b_desc)  > 0 then 1 else 0 end)::real as f_desc,
           avg(case when b.b_tasks = '' then 0 when position(k.tok in b.b_tasks) > 0 then 1 else 0 end)::real as f_tasks
    from b
    cross join k
...
grant execute on function public.submissions_search_scored(text, boolean, boolean) to authenticated;
...
grant execute on function public.submissions_search(text, text, text[], text, int, int, boolean, boolean) to authenticated;
